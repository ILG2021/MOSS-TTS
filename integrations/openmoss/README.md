...
| `--codec-cpu`         | run only AudioTokenizer codec on CPU; audio embeds and LM heads stay on GPU |
...
`--no-flash-attn`, `--skip-codec`, and `--codec-cpu` flags as the CLI. Generations are serialized through a single mutex (libllama
