...
struct GenerateRequest {
    ...
    // Optional runtime adapter selected by the server.  The base model and
    // frozen MOSS auxiliary tensors stay shared across every adapter.
    std::string                       lora;
...
// Parallel-server entry point.  `ctx` owns independent KV/sampler state while
// sharing Model's immutable backbone and auxiliary weights.
