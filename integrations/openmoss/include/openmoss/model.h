...
struct LoadOptions {
    ...
    // Per-context KV cache types understood by llama.cpp. Quantized caches are
    // especially useful when several inference contexts share an 8 GB GPU.
    ...
    // Keep audio embeddings/LM heads on the selected accelerator, but place
    // the much larger AudioTokenizer codec weights and graphs on CPU.
...
public:
    ...
    // Create an additional inference context sharing the same immutable
    // backbone weights.  The caller owns the returned context and must free it
    // with llama_free().  This is the unit used by the server's parallel slots.
    llama_context * create_backbone_context() const;

    // Runtime LoRA registry.  Adapters are attached to the shared llama_model
    // once and can then be selected independently on each llama_context.
    void load_lora(const std::string & name, const std::string & path);
    bool has_lora(const std::string & name) const;
    void set_lora(llama_context * ctx, const std::string & name, float scale = 1.0f) const;
    void clear_lora(llama_context * ctx) const;
    ...
    Aux * codec_aux() const { return m_codec_aux.get(); }

    // Aux GGML graphs reuse allocators and codec graph objects.  Contexts may
    // run libllama concurrently, but these short sections must be serialized.
    std::mutex & aux_compute_mutex() const { return m_aux_compute_mu; }
    std::mutex & codec_mutex() const { return m_codec_mu; }
...
private:
    ...
    LoadOptions     m_load_options;
    std::unordered_map<std::string, llama_adapter_lora *> m_loras;
    mutable std::mutex m_lora_mu;
    mutable std::mutex m_aux_compute_mu;
    mutable std::mutex m_codec_mu;

    std::unique_ptr<Aux>                m_aux;
    std::unique_ptr<Aux>                m_codec_aux;
