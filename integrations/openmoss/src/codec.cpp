...
CodecGraphs::CodecGraphs(Model & owner) : m_owner(owner) {
    ...
    auto * aux = m_owner.codec_aux();
    if (!aux || !aux->backend) {
        throw std::runtime_error("CodecGraphs: codec backend not initialised");
    }
...
std::vector<float> CodecGraphs::decode(const int32_t * codes,
    ...
    if (!m_owner.codec_loaded()) {
        throw std::runtime_error("CodecGraphs::decode: codec tensors not loaded "
                                 "(don't pass --skip-codec when waveform output is required)");
    }
    auto * aux = m_owner.codec_aux();
    if (!aux || !aux->backend) {
        throw std::runtime_error("CodecGraphs::decode: codec backend not initialised");
    }
...
std::vector<float> CodecGraphs::stream_push(StreamState & st,
                                             ...
                                             int32_t T_audio) {
    if (!m_owner.codec_loaded()) {
        throw std::runtime_error("CodecGraphs::stream_push: codec tensors not loaded");
    }
    auto * aux = m_owner.codec_aux();
    if (!aux || !aux->backend) {
        throw std::runtime_error("CodecGraphs::stream_push: codec backend not initialised");
    }
...
std::vector<int32_t> CodecGraphs::encode(const float * waveform,
    ...
    auto * aux = m_owner.codec_aux();
    if (!aux || !aux->backend) {
        throw std::runtime_error("CodecGraphs::encode: codec backend not initialised");
    }
...
Model::~Model() {
    // Destroy wrappers/graphs that may retain model or aux tensor pointers
    // before releasing their backing storage. This also makes failed partial
    // loads and embedded-library use clean, rather than relying on process exit.
    m_codec.reset();
    m_dit.reset();
    m_dac.reset();
    m_tokenizer.reset();
    ...
    if (m_backbone_model) {
        // llama.cpp also releases every LoRA adapter associated with the model.
...
std::vector<int32_t> codec_encode(Model & model,
                                   ...
                                   int32_t &     n_vq_out,
                                   int32_t &     t_audio_out) {
    std::lock_guard<std::mutex> codec_lock(model.codec_mutex());
