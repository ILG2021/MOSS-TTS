...
// With --parallel N, the delay family uses N independent llama contexts that
// share immutable model/LoRA weights; shared aux and codec workspaces retain
// their own narrow locks.
//
// API
// ---
//   GET  /health                       → "ok\n"  (200)
...
//     "lora":             str,       // adapter name preloaded by --lora
...
public:
    ...
    public:
        Lease() = default;
        Lease(InferencePool * pool, InferenceSlot * slot) : pool_(pool), slot_(slot) {}
        ...
        ~Lease() { if (pool_ && slot_) pool_->release(slot_); }
        llama_context * ctx() const { return slot_->ctx; }
    private:
        InferencePool * pool_ = nullptr;
        InferenceSlot * slot_ = nullptr;
    };

    InferencePool(openmoss::Model & model, int count) {
        count = std::max(1, count);
        slots_.reserve(size_t(count));
        slots_.push_back({model.backbone_ctx(), false});
        for (int i = 1; i < count; ++i) {
            llama_context * ctx = model.create_backbone_context();
            if (!ctx) throw std::runtime_error("failed to create parallel llama context");
            slots_.push_back({ctx, true});
        }
        for (auto & slot : slots_) idle_.push(&slot);
    }

    ~InferencePool() {
        for (auto & slot : slots_) if (slot.owned) llama_free(slot.ctx);
    }

    Lease acquire() {
        std::unique_lock<std::mutex> lock(mu_);
        ready_.wait(lock, [&] { return !idle_.empty(); });
        InferenceSlot * slot = idle_.front();
        idle_.pop();
...
private:
    void release(InferenceSlot * slot) {
        {
...
[[noreturn]] void usage(int code) {
    std::fprintf(stderr,
        ...
        "  --cache-type-k TYPE    KV key cache: f16, q8_0 or q4_0 (default: f16)\n"
        "  --cache-type-v TYPE    KV value cache: f16, q8_0 or q4_0 (default: f16)\n"
        "  --parallel N           independent inference contexts sharing weights (default: 1)\n"
        "  --lora NAME=PATH       preload a GGUF LoRA; repeat for multiple adapters\n"
        ...
        "  --codec-cpu            run only AudioTokenizer codec on CPU; keep\n"
        "                         audio embeddings and LM heads on GPU\n"
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (k == "--cache-type-k")   cache_type_k = next();
        else if (k == "--cache-type-v")   cache_type_v = next();
        else if (k == "--parallel")       parallel     = std::atoi(next().c_str());
        else if (k == "--lora") {
            const std::string value = next();
            const size_t eq = value.find('=');
            if (eq == std::string::npos || eq == 0 || eq + 1 >= value.size()) {
                std::fprintf(stderr, "--lora expects NAME=PATH, got: %s\n", value.c_str());
                usage(2);
            }
            const std::string name = value.substr(0, eq);
            const auto duplicate = std::find_if(
                lora_paths.begin(), lora_paths.end(),
                [&](const auto & item) { return item.first == name; });
            if (duplicate != lora_paths.end()) {
                std::fprintf(stderr, "duplicate --lora name: %s\n", name.c_str());
                usage(2);
            }
            lora_paths.emplace_back(name, value.substr(eq + 1));
        }
        else if (k == "--no-flash-attn")  flash_attn   = false;
        else if (k == "--skip-codec")     skip_codec   = true;
        else if (k == "--codec-cpu")      codec_cpu    = true;
        else if (k == "--template") {
    ...
    if (parallel < 1) {
        std::fprintf(stderr, "--parallel must be >= 1\n");
        return 2;
    }
    const auto valid_cache_type = [](const std::string & value) {
        return value == "f16" || value == "q8_0" || value == "q4_0";
    };
    if (!valid_cache_type(cache_type_k) || !valid_cache_type(cache_type_v)) {
        std::fprintf(stderr, "cache type must be f16, q8_0 or q4_0\n");
        return 2;
    }
    if (!flash_attn && cache_type_v != "f16") {
        std::fprintf(stderr,
            "quantized V cache requires flash attention; use --cache-type-v f16 "
            "or remove --no-flash-attn\n");
        return 2;
    }
    for (const auto & item : lora_paths) {
        std::error_code ec;
        if (!std::filesystem::is_regular_file(item.second, ec)) {
            std::fprintf(stderr, "LoRA '%s' not found: %s\n",
    ...
    lo.cache_type_k = cache_type_k;
    lo.cache_type_v = cache_type_v;
    lo.skip_codec   = skip_codec;
    lo.codec_cpu    = codec_cpu;
    auto model = openmoss::Model::load(model_path, lo);
    for (const auto & item : lora_paths) {
        model->load_lora(item.first, item.second);
        std::fprintf(stderr, "[server] loaded LoRA '%s' from %s\n",
                     item.first.c_str(), item.second.c_str());
    }
    std::fprintf(stderr,
    ...
    if (parallel > 1 && model->dims().arch != openmoss::Arch::TTSDelay) {
        std::fprintf(stderr,
            "--parallel > 1 is currently supported only for moss_tts_delay; "
            "local/soundeffect retain shared mutable graphs\n");
        return 2;
    }
    InferencePool inference_pool(*model, parallel);
    std::fprintf(stderr, "[server] inference slots: %zu\n", inference_pool.size());

    auto parse_lora = [&](const json & body, openmoss::GenerateRequest & req,
                          std::string & err) -> bool {
        auto it = body.find("lora");
        if (it == body.end() || it->is_null()) return true;
        if (!it->is_string()) {
            err = "'lora' must be a configured adapter name";
            return false;
        }
        req.lora = it->get<std::string>();
        req.lora_scale = jget(body, "lora_scale", 1.0f);
        if (!model->has_lora(req.lora)) {
            err = "unknown LoRA adapter '" + req.lora + "'";
            return false;
        }
        return true;
    };

    auto run_tts = [&](const openmoss::GenerateRequest & req,
                       openmoss::StreamCallback cb = {}) {
        auto slot = inference_pool.acquire();
        if (req.lora.empty()) model->clear_lora(slot.ctx());
        else model->set_lora(slot.ctx(), req.lora, req.lora_scale);
        return openmoss::generate_with_context(*model, slot.ctx(), req, std::move(cb));
    };
    ...
    svr.Get("/info", [&](const httplib::Request &, httplib::Response & rs) {
        ...
        json info = {
            ...
            {"n_ctx",             lo.n_ctx},
            {"n_batch",           lo.n_batch},
            {"cache_type_k",      lo.cache_type_k},
            {"cache_type_v",      lo.cache_type_v},
            {"codec_cpu",         lo.codec_cpu},
            {"requests_served",   uint64_t(n_requests.load())},
        };
        info["lora_adapters"] = json::array();
        for (const auto & item : lora_paths) {
            info["lora_adapters"].push_back(item.first);
        }
    ...
    auto stream_tts = [&](openmoss::GenerateRequest req, uint64_t req_id,
        ...
        rs.set_chunked_content_provider("audio/pcm",
            [&run_tts, req, req_id, t0, rate, chans]
    ...
    svr.Post("/tts", [&](const httplib::Request & rq, httplib::Response & rs) {
        ...
        {
            std::string lora_err;
            if (!parse_lora(body, req, lora_err)) {
                return send_text_error(rs, 400, lora_err);
            }
        }
        ...
        if (want_stream && inference_pool.size() > 1) {
            return send_text_error(rs, 400,
                "streaming is disabled with --parallel > 1 until codec graph "
