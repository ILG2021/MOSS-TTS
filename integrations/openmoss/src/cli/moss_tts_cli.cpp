...
[[noreturn]] void usage(int code) {
    std::fprintf(stderr,
        ...
        "  --codec-cpu            Run only the AudioTokenizer codec on CPU. The\n"
        "                         backbone, audio embeddings and LM heads stay on GPU.\n"
        "\n"
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (k == "--codec-cpu")       a.codec_cpu  = true;
    ...
    if (a.vk_f32 && std::getenv("GGML_VK_DISABLE_F16") == nullptr) {
#ifdef _WIN32
        _putenv_s("GGML_VK_DISABLE_F16", "1");
#else
        setenv("GGML_VK_DISABLE_F16", "1", /*overwrite=*/0);
#endif
    }
