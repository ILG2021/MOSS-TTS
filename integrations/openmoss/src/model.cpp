...
ggml_type kv_cache_type(const std::string & name) {
    if (name == "f16")  return GGML_TYPE_F16;
    if (name == "q8_0") return GGML_TYPE_Q8_0;
    if (name == "q4_0") return GGML_TYPE_Q4_0;
    throw std::runtime_error(
        "unsupported KV cache type '" + name + "' (expected f16, q8_0 or q4_0)");
}
...
bool Model::codec_loaded() const {
    if (!m_codec_aux) return false;
    return m_codec_aux->tensors.count("moss.codec.quantizer.q.0.codebook.weight") > 0;
}
...
std::unique_ptr<Model> Model::load(const std::string & gguf_path, const LoadOptions & opts) {
    ...
    cp.type_k          = kv_cache_type(opts.cache_type_k);
    cp.type_v          = kv_cache_type(opts.cache_type_v);
    ...
    LoadSpec codec_spec; codec_spec.path = sidecar_path;
    ...
    if (!sc_spec.gctx) {
        ...
        if (include_codec) {
            codec_spec.gctx = sc_spec.gctx;
            codec_spec.meta_ctx = sc_spec.meta_ctx;
            auto first_codec = std::stable_partition(
                sc_spec.wanted.begin(), sc_spec.wanted.end(),
                [](const auto & item) {
                    return item.second.rfind("moss.codec.", 0) != 0;
                });
            codec_spec.wanted.assign(first_codec, sc_spec.wanted.end());
            sc_spec.wanted.erase(first_codec, sc_spec.wanted.end());
            if (!codec_spec.wanted.empty()) {
                self->m_codec_aux = std::make_unique<Aux>();
            }
        }
    ...
    if (picked_dev) {
        ...
        // No GPU: keep the backbone and auxiliary projections on CPU.
    ...
    if (self->m_codec_aux) {
        if (picked_dev && !opts.codec_cpu) {
            self->m_codec_aux->backend = ggml_backend_dev_init(picked_dev, nullptr);
        } else {
            self->m_codec_aux->backend = ggml_backend_init_by_type(
                GGML_BACKEND_DEVICE_TYPE_CPU, nullptr);
        }
        if (!self->m_codec_aux->backend) {
            throw std::runtime_error("Model::load: no codec GGML backend available");
        }
        std::fprintf(stderr, "Model::load: codec backend = %s\n",
                     ggml_backend_name(self->m_codec_aux->backend));
    }
    ...
    if (self->m_codec_aux) {
        ggml_init_params ip{};
        ip.mem_size   = ggml_tensor_overhead() * (codec_spec.wanted.size() + 16);
        ip.mem_buffer = nullptr;
        ip.no_alloc   = true;
        self->m_codec_aux->ctx = ggml_init(ip);
        if (!self->m_codec_aux->ctx) {
            throw std::runtime_error("Model::load: ggml_init for codec ctx failed");
        }
    }
    ...
    try {
        ...
        if (self->m_codec_aux) {
            stage_tensor_descriptors(codec_spec, *self->m_codec_aux, codec_bytes);
        }
        ...
        if (self->m_codec_aux) {
            self->m_codec_aux->buffer = ggml_backend_alloc_ctx_tensors(
                self->m_codec_aux->ctx, self->m_codec_aux->backend);
            if (!self->m_codec_aux->buffer) {
                throw std::runtime_error("Model::load: failed to allocate codec backend buffer ("
                                         + std::to_string(codec_bytes / (1024 * 1024)) + " MiB)");
            }
        }

        if (sc_spec.gctx) upload_tensor_data(sc_spec, *self->m_aux);
        upload_tensor_data(bb_spec, *self->m_aux);
        if (self->m_codec_aux) upload_tensor_data(codec_spec, *self->m_codec_aux);
    } catch (...) {
        if (sc_spec.gctx) gguf_free(sc_spec.gctx);
        if (sc_spec.meta_ctx) ggml_free(sc_spec.meta_ctx);
        gguf_free(bb_spec.gctx);
...
llama_context * Model::create_backbone_context() const {
    if (!m_backbone_model) return nullptr;
    llama_context_params cp = llama_context_default_params();
    cp.n_ctx           = uint32_t(m_load_options.n_ctx);
    cp.n_batch         = uint32_t(m_load_options.n_batch);
    cp.n_ubatch        = uint32_t(m_load_options.n_batch);
    cp.n_threads       = m_load_options.n_threads;
    cp.n_threads_batch = m_load_options.n_threads;
    cp.embeddings      = true;
    cp.pooling_type    = LLAMA_POOLING_TYPE_NONE;
    cp.flash_attn_type = m_load_options.flash_attn
        ? LLAMA_FLASH_ATTN_TYPE_ENABLED
        : LLAMA_FLASH_ATTN_TYPE_DISABLED;
    cp.type_k          = kv_cache_type(m_load_options.cache_type_k);
    cp.type_v          = kv_cache_type(m_load_options.cache_type_v);
    return llama_init_from_model(m_backbone_model, cp);
}

void Model::load_lora(const std::string & name, const std::string & path) {
    if (name.empty()) throw std::runtime_error("load_lora: adapter name is empty");
    std::lock_guard<std::mutex> lock(m_lora_mu);
    if (m_loras.count(name)) {
        throw std::runtime_error("load_lora: duplicate adapter name '" + name + "'");
    }
    llama_adapter_lora * adapter = llama_adapter_lora_init(m_backbone_model, path.c_str());
    if (!adapter) {
        throw std::runtime_error("load_lora: failed to load '" + path + "'");
    }
    m_loras.emplace(name, adapter);
}

bool Model::has_lora(const std::string & name) const {
    std::lock_guard<std::mutex> lock(m_lora_mu);
    return m_loras.count(name) != 0;
}

void Model::set_lora(llama_context * ctx, const std::string & name, float scale) const {
    std::lock_guard<std::mutex> lock(m_lora_mu);
    auto it = m_loras.find(name);
    if (it == m_loras.end()) {
        throw std::runtime_error("unknown LoRA adapter '" + name + "'");
    }
    llama_adapter_lora * adapter = it->second;
    float adapter_scale = scale;
    if (llama_set_adapters_lora(ctx, &adapter, 1, &adapter_scale) != 0) {
        throw std::runtime_error("failed to apply LoRA adapter '" + name + "'");
    }
}

void Model::clear_lora(llama_context * ctx) const {
    if (llama_set_adapters_lora(ctx, nullptr, 0, nullptr) != 0) {
        throw std::runtime_error("failed to clear LoRA adapters");
    }
}
...
std::vector<float> Model::compute_input_embeddings(const int32_t * prompt_grid,
                                                    int32_t         n_pos) const {
    std::lock_guard<std::mutex> compute_lock(m_aux_compute_mu);
