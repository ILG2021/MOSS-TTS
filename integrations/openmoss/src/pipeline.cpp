...
GenerateResult generate(Model & model,
                        const GenerateRequest & req,
                        StreamCallback cb) {
    return generate_with_context(model, model.backbone_ctx(), req, std::move(cb));
}

GenerateResult generate_with_context(Model & model,
                                     llama_context * backbone_ctx,
                                     const GenerateRequest & req,
                                     StreamCallback cb) {
    const auto & d = model.dims();
    ...
    {
        const int32_t n_ctx = int32_t(llama_n_ctx(backbone_ctx));
        std::string   because;
        if (T_ref > 0) {
            char buf[160];
    ...
    auto prefill_embeds = model.compute_input_embeddings(grid.data(), prompt_len);
    {
        ...
        llama_memory_clear(llama_get_memory(backbone_ctx), /*data=*/true);
        llama_decode_embeddings(backbone_ctx,
                                prefill_embeds.data(),
    ...
    for (; step < req.max_new_tokens; ++step) {
        const float * text_logits = llama_get_logits_ith(backbone_ctx, -1);
        const float * hidden_vec  = llama_get_embeddings_ith(backbone_ctx, -1);
