...
from accelerate.utils import DistributedType, broadcast_object_list, enable_fsdp_ram_efficient_loading, set_seed
...
def add_lora_arguments(parser):
    ...
    group.add_argument("--lora-target-modules", default=DEFAULT_TARGETS,
                       help="Comma-separated linear-layer suffixes within language_model.")
    group.add_argument("--lora-resume-adapter", default=None,
                       help="Load adapter weights for further training; optimizer/scheduler restart.")
    group.add_argument("--resume-from-checkpoint", default=None,
                       help="Restore a complete checkpoint, including its training configuration.")
    group.add_argument("--save-steps", type=int, default=0,
                       help="Save every N optimizer-step boundaries; 0 saves only at epoch end.")


def validate_lora_args(args):
    if args.save_steps < 0:
        raise ValueError("--save-steps must be >= 0.")
    if args.resume_from_checkpoint and args.lora_resume_adapter:
        raise ValueError("Choose either full checkpoint resume or adapter-only loading.")
...
def resolve_targets(model, spec):
    from torch import nn

    selectors = {item.strip() for item in spec.split(",") if item.strip()}
    if not selectors:
        raise ValueError("LoRA target modules cannot be empty.")
    matches = {selector: [] for selector in selectors}
    for name, module in model.named_modules():
        if name.startswith("language_model.") and isinstance(module, nn.Linear):
            for selector in selectors:
                if name == selector or name.endswith("." + selector):
                    matches[selector].append(name)
    missing = sorted(selector for selector, names in matches.items() if not names)
    if missing:
        raise ValueError("No backbone linear layers matched: " + ", ".join(missing))
...
def attach_lora(model, args):
    try:
        ...
        raise ImportError('Install LoRA dependencies with: pip install -e ".[finetune-lora]"') from exc

    adapter_path = args.resume_from_checkpoint or args.lora_resume_adapter
    if adapter_path:
        return PeftModel.from_pretrained(model, adapter_path, is_trainable=True)
    # Generic PeftModel preserves the [batch, sequence, channels] input and
    # channelwise_loss_weight. Do not use the standard CAUSAL_LM task wrapper.
    config = LoraConfig(
        r=args.lora_r,
        lora_alpha=args.lora_alpha,
        lora_dropout=args.lora_dropout,
        target_modules=resolve_targets(model, args.lora_target_modules),
        bias="none",
    )
...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Standalone LoRA finetuning for MossTTSDelay-family tasks."
    )
    ...
    parser.add_argument("--output-dir", type=str, default="output/moss_tts_lora")
...
def read_resume_checkpoint(args):
    """Restore training settings, retaining only runtime/output choices from CLI."""
    if not args.resume_from_checkpoint:
        return None
    if args.lora_resume_adapter:
        raise ValueError("Choose either full checkpoint resume or adapter-only loading.")
    root = Path(args.resume_from_checkpoint)
    marker = root / "trainer_state.json"
    if not marker.is_file() or not (root / "training_state").is_dir():
        raise ValueError(f"Not a completed full checkpoint: {root}")
    with marker.open(encoding="utf-8") as handle:
        progress = json.load(handle)
    if progress.get("format_version") != 1:
        raise ValueError("Unsupported checkpoint format.")
    with (root / "finetune_args.json").open(encoding="utf-8") as handle:
...
def configure_checkpoint_hooks(accelerator):
    # Adapter weights are saved at the checkpoint root and loaded before prepare.
    # Sharded backends must retain their native distributed model state.
    if accelerator.distributed_type not in {DistributedType.FSDP, DistributedType.DEEPSPEED}:
        def skip_model_save(models, weights, output_dir):
            weights.clear()

        def skip_model_load(models, input_dir):
            models.clear()

        accelerator.register_save_state_pre_hook(skip_model_save)
        accelerator.register_load_state_pre_hook(skip_model_load)
...
def save_checkpoint(
    ...
    existing = [output_dir.exists() if accelerator.is_main_process else None]
    broadcast_object_list(existing)
    if existing[0]:
        raise FileExistsError(f"Refusing to overwrite checkpoint: {output_dir}")
    if accelerator.is_main_process:
        output_dir.mkdir(parents=True, exist_ok=True)
    unwrapped_model = accelerator.unwrap_model(model)
    # DDP adapters are replicated; avoid gathering the frozen base weights.
    if accelerator.distributed_type not in {
        DistributedType.FSDP, DistributedType.DEEPSPEED,
    }:
        state_dict = unwrapped_model.state_dict() if accelerator.is_main_process else {}
    else:
    ...
    accelerator.wait_for_everyone()
    accelerator.save_state(str(output_dir / "training_state"))
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        # Written last: an interrupted save is never accepted as resumable.
        temporary = output_dir / "trainer_state.json.tmp"
...
def main(argv=None) -> None:
    ...
    resume_progress = read_resume_checkpoint(args)
    validate_args(args)
    ...
    if accelerator.distributed_type == DistributedType.FSDP:
        ...
        if not plugin.use_orig_params:
            raise ValueError("LoRA requires fsdp_use_orig_params: true (mixed frozen/trainable parameters).")
    ...
    accelerator.print(f"[{format_timestamp()}] [lora] global_batch_size={global_batch_formula}")

    # Let Accelerate shard batches, including JSONLs produced by distributed preprocessing.
    train_paths = resolve_jsonl_paths(args.train_jsonl)
    fingerprint = dataset_fingerprint(train_paths)
    if resume_progress:
        if (resume_progress["dataset_fingerprint"] != fingerprint
                or resume_progress["world_size"] != accelerator.num_processes
                or resume_progress["distributed_type"] != str(accelerator.distributed_type)):
            raise ValueError("Full resume requires unchanged data, process count and distributed backend.")
    records = [record for path in train_paths for record in load_jsonl(path)]
    if not records:
    ...
    configure_checkpoint_hooks(accelerator)
...
def _training_loop(
    ...
    start_epoch = resume_progress["epoch"] if resume_progress else 0
    skip_batches = resume_progress["next_batch"] if resume_progress else 0
    completed_epochs = 0
    ...
    def checkpoint(path, epoch, next_batch):
        ...
        save_checkpoint(
            accelerator=accelerator, model=model, model_path=args.model_path,
            codec_path=args.codec_path, output_dir=path, train_args=train_args_to_save,
            progress={
                "global_step": global_step, "epoch": epoch, "next_batch": next_batch,
                "world_size": accelerator.num_processes,
                "distributed_type": str(accelerator.distributed_type),
                "dataset_fingerprint": train_args_to_save["dataset_fingerprint"],
            },
        )

    if global_step >= max_train_steps:
        accelerator.print("Checkpoint has already reached the configured training target.")
        return
    if not 0 <= skip_batches < len(train_dataloader):
        raise ValueError("Invalid checkpoint batch position.")
    if skip_batches % args.gradient_accumulation_steps:
        raise ValueError("Checkpoint is not at a completed gradient accumulation boundary.")

    steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    num_epochs = math.ceil(max_train_steps / steps_per_epoch)
    for epoch in range(start_epoch, max(args.num_epochs, num_epochs)):
        train_dataloader.set_epoch(epoch)
        # Worker/base seeds must not advance the model's restored RNG stream.
        train_dataloader.generator.manual_seed(args.seed + epoch)
        epoch_loader = accelerator.skip_first_batches(train_dataloader, skip_batches) if skip_batches else train_dataloader
        epoch_loader.set_epoch(epoch)
        model.train()
        for batch_index, batch in enumerate(epoch_loader, start=skip_batches):
        ...
        checkpoint(checkpoint_dir, epoch, batch_index + 1)
        skip_batches = 0
